...
export class AdvancedComponent<E extends EventList = Record<string, any>>
  ...
  private readonly _api: AdvancedComponentApi
  ...
  render(): void {
    ...
    switch (this.props.kind) {
      case 'SpeedDial':
        this._renderSpeedDial(schema)
        break
      case 'Dock':
        this._renderDock(schema)
        break
      case 'Carousel':
      case 'Galleria':
        this._renderGallery(schema)
        break
      case 'ImagePreview':
        this._renderImagePreview(schema)
        break
      case 'ImageCompare':
        this._renderImageCompare(schema)
        break
      case 'Skeleton':
        this._renderSkeleton(schema)
        break
      case 'ProgressBar':
        this._renderProgressBar(schema)
        break
      case 'ProgressSpinner':
        this._renderProgressSpinner(schema)
        break
      case 'MeterGroup':
        this._renderMeterGroup(schema)
        break
      case 'Knob':
        this._renderKnob(schema)
        break
      case 'ToggleSwitch':
        this._renderToggleSwitch(schema)
        break
      case 'RadioButton':
        this._renderRadioButton(schema)
        break
      case 'Rating':
        this._renderRating(schema)
        break
      case 'SelectButton':
        this._renderSelectButton(schema)
        break
      case 'Drawer':
        this._renderOverlay(schema)
        break
      case 'Message':
        this._renderMessage(schema)
        break
      case 'BlockUI':
        this._renderBlockUi(schema)
        break
      case 'Accordion':
      case 'Fieldset':
        this._renderDisclosure(schema)
        break
      case 'Tabs':
        this._renderTabs(schema)
        break
      case 'Stepper':
        this._renderStepper(schema)
        break
    }
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderSpeedDial(schema: NovaSchema): void {
    ...
    items.forEach((item, index) => {
      ...
      const active = index === this._hoveredIndex || index === this._pressedIndex
      this._pushRoundAction(schema, x - 18, y - 18, 36, item, active)
    })
    this._pushRoundAction(schema, cx - 24, cy - 24, 48, { label: '+', color: this.props.accentColor ?? '#2563eb' }, true)
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderDock(schema: NovaSchema): void {
    const items = this.props.items
    const gap = 12
    const size = 42
    ...
    items.forEach((item, index) => {
      const active = index === this._hoveredIndex
      const itemSize = active ? size + 10 : size
      this._pushRoundAction(schema, x - (itemSize - size) / 2, (this.height - itemSize) / 2, itemSize, item, active)
      x += size + gap
    })
  }
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderGallery(schema: NovaSchema): void {
    const items = this.props.items
    const active = items[Math.min(items.length - 1, this.props.activeIndex)] ?? items[0]
    const pad = 12
    const thumb = 34

    this._pushRect(schema, pad, pad, this.width - pad * 2, this.height - thumb - pad * 3, active?.color ?? '#60a5fa', 'media', 12)
    pushText(schema, active?.label ?? (this.props.title || this.props.kind), pad + 12, pad + 10, this.width - pad * 4, 26, this._textStyle({ color: '#ffffff', fontSize: 16, fontWeight: '700' }))

    items.forEach((item, index) => {
      const x = pad + index * (thumb + 8)
      this._pushRect(schema, x, this.height - pad - thumb, thumb, thumb, item.color ?? '#cbd5e1', 'thumbnail', 8, index === this.props.activeIndex ? '#2563eb' : '#dbe4ef')
    })
  }
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderImagePreview(schema: NovaSchema): void {
    this._pushRect(schema, 12, 12, this.width - 24, this.height - 24, this.props.parts?.image?.background ?? '#bae6fd', 'image', 14)
    if (this.props.image) {
      pushIcon(schema, this.props.image, 20, 20, Math.max(24, Math.min(this.width, this.height) - 40))
    }
    this._pushRect(schema, 12, 12, this.width - 24, this.height - 24, 'rgba(15,23,42,0.28)', 'previewMask', 14)
    pushText(schema, this.props.title || 'Preview', 24, this.height / 2 - 12, this.width - 48, 24, this._textStyle({ color: '#ffffff', fontWeight: '700' }), { align: 'center' })
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderImageCompare(schema: NovaSchema): void {
    const split = (clamp(Number(this.props.value), 0, 100) / 100) * this.width
    this._pushRect(schema, 10, 10, this.width - 20, this.height - 20, this.props.parts?.before?.background ?? '#bfdbfe', 'before', 14)
    this._pushRect(schema, 10, 10, Math.max(0, split - 10), this.height - 20, this.props.parts?.after?.background ?? '#7dd3fc', 'after', 14)
    schema.push({ type: 'line', x1: split, y1: 14, x2: split, y2: this.height - 14, styles: { color: '#ffffff', width: 3 } })
    this._pushRect(schema, split - 10, this.height / 2 - 16, 20, 32, '#ffffff', 'handle', 999, '#0ea5e9')
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderSkeleton(schema: NovaSchema): void {
    const part = this._part('shimmer')
    this._pushRect(schema, 16, 16, this.width * 0.42, 14, '#cbd5e1', 'line', 999)
    this._pushRect(schema, 16, 42, this.width - 48, 12, '#dbe4ef', 'line', 999)
    this._pushRect(schema, 16, 66, this.width * 0.62, 12, '#dbe4ef', 'line', 999)
    this._pushRect(schema, this.width * 0.16, 0, this.width * 0.28, this.height, part.background ?? 'rgba(255,255,255,0.42)', 'shimmer', 0)
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderProgressBar(schema: NovaSchema): void {
    const pct = this._percent()
    this._pushRect(schema, 2, 2, Math.max(0, (this.width - 4) * pct), this.height - 4, this.props.accentColor ?? '#2563eb', 'value', 999)
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderProgressSpinner(schema: NovaSchema): void {
    ...
    const angle = -Math.PI / 2 + TWO_PI * this._percent()
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderMeterGroup(schema: NovaSchema): void {
    ...
    for (const item of this.props.items) {
      ...
      this._pushRect(schema, x, y, w, 18, item.color ?? '#94a3b8', 'meter', 999)
      x += w
    }
    this.props.items.forEach((item, index) => {
      const lx = 14 + index * 86
      schema.push({ type: 'circle', x: lx + 5, y: 52, radius: 5, styles: { background: item.color ?? '#94a3b8' } })
      pushText(schema, item.label ?? '', lx + 16, 43, 66, 18, this._textStyle({ fontSize: 11 }))
    })
  }
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderKnob(schema: NovaSchema): void {
    const cx = this.width / 2
    const cy = this.height / 2
    ...
    pushText(schema, `${Math.round(pct * 100)}%`, cx - 28, cy - 10, 56, 20, this._textStyle({ fontWeight: '700' }), { align: 'center' })
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderToggleSwitch(schema: NovaSchema): void {
    const checked = this.props.checked
    const h = this.height
    const knob = Math.max(18, h - 8)
    this._pushRect(schema, 0, 0, this.width, h, checked ? this.props.accentColor ?? '#2563eb' : '#cbd5e1', 'track', 999)
    this._pushRect(schema, checked ? this.width - knob - 4 : 4, 4, knob, knob, '#ffffff', 'thumb', 999)
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderRadioButton(schema: NovaSchema): void {
    ...
    pushText(schema, this.props.text, size + 10, 0, this.width - size - 10, this.height, this._textStyle())
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderRating(schema: NovaSchema): void {
    ...
    for (let index = 0; index < count; index += 1) {
      const x = 8 + index * 28
      this._pushStar(schema, x + 10, this.height / 2, 10, index < active ? this.props.accentColor ?? '#f59e0b' : '#cbd5e1')
    }
  }
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderSelectButton(schema: NovaSchema): void {
    const items = this.props.items
    const count = Math.max(1, items.length)
    const w = this.width / count
    const activeIndex = Math.max(0, items.findIndex(item => item.value === this.props.value))

    this._pushRect(schema, activeIndex * w + 3, 3, w - 6, this.height - 6, '#ffffff', 'indicator', 8, '#dbe4ef')
    items.forEach((item, index) => {
      pushText(schema, item.label ?? item.value ?? '', index * w + 8, 0, w - 16, this.height, this._textStyle({
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderOverlay(schema: NovaSchema): void {
    const isDrawer = this.props.kind === 'Drawer'
    this._pushRect(schema, 0, 0, this.width, this.height, 'rgba(15,23,42,0.10)', 'mask', 14)
    ...
    this._pushRect(schema, x, y, w, h, '#ffffff', 'surface', 12, '#cbd5e1')
    pushText(schema, this.props.title, x + 16, y + 14, w - 32, 24, this._textStyle({ fontWeight: '700', fontSize: 15 }))
    pushText(schema, this.props.subtitle, x + 16, y + 44, w - 32, 22, this._textStyle({ color: '#64748b', fontSize: 12 }))
    this._pushRect(schema, x + 16, y + h - 38, 72, 24, this.props.accentColor ?? '#2563eb', 'action', 8)
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderMessage(schema: NovaSchema): void {
    ...
    pushText(schema, this.props.text, 40, 0, this.width - 52, this.height, this._textStyle({ color: tone.color, fontWeight: '600' }))
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderBlockUi(schema: NovaSchema): void {
    pushText(schema, this.props.title, 18, 18, this.width - 36, 24, this._textStyle({ fontWeight: '700' }))
    pushText(schema, this.props.subtitle, 18, 48, this.width - 36, 38, this._textStyle({ color: '#64748b', fontSize: 12 }))
    if (this.props.blocked) {
      this._pushRect(schema, 0, 0, this.width, this.height, 'rgba(15,23,42,0.18)', 'mask', 12)
      this._pushRect(schema, this.width / 2 - 46, this.height / 2 - 14, 92, 28, '#ffffff', 'maskLabel', 999)
      pushText(schema, 'Blocked', this.width / 2 - 38, this.height / 2 - 10, 76, 20, this._textStyle({ fontWeight: '700' }), { align: 'center' })
    }
  }
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderDisclosure(schema: NovaSchema): void {
    pushText(schema, this.props.title, 16, 12, this.width - 48, 22, this._textStyle({ fontWeight: '700' }))
    pushText(schema, this.props.expanded ? 'v' : '>', this.width - 34, 12, 18, 22, this._textStyle({ fontWeight: '700' }), { align: 'center' })
    ...
    this.props.items.forEach((item, index) => {
      ...
      pushText(schema, item.label ?? item.value ?? '', 34, y, this.width - 48, 18, this._textStyle({ color: '#475569', fontSize: 12 }))
    })
  }
   ...
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderTabs(schema: NovaSchema): void {
    const items = this.props.items
    const count = Math.max(1, items.length)
    const w = this.width / count
    const active = Math.min(count - 1, this.props.activeIndex)
    this._pushRect(schema, active * w + 12, 10, w - 24, 28, '#eff6ff', 'tabActive', 8)
    items.forEach((item, index) => {
      pushText(schema, item.label ?? '', index * w + 8, 12, w - 16, 24, this._textStyle({ color: index === active ? '#1d4ed8' : '#64748b', fontWeight: index === active ? '700' : '500' }), { align: 'center' })
    })
    this._pushRect(schema, 18, 56, this.width - 36, 56, '#f8fafc', 'panel', 10, '#e2e8f0')
    pushText(schema, items[active]?.label ?? 'Tab', 32, 72, this.width - 64, 20, this._textStyle({ fontWeight: '700' }))
  }

  /**
   * Выполняет отрисовку AdvancedComponent.
   */
  private _renderStepper(schema: NovaSchema): void {
    const items = this.props.items
    const count = Math.max(1, items.length)
    const y = 34
    const left = 26
    ...
    items.forEach((item, index) => {
      ...
      pushText(schema, item.label ?? '', x - 38, 56, 76, 18, this._textStyle({ color: '#475569', fontSize: 11 }), { align: 'center' })
    })
  }
  ...
  private _setupEvents(): void {
    this.on('mousemove', (event) => {
      ...
      const next = this._indexFromEvent(event)
      if (next !== this._hoveredIndex && next >= 0) {
    ...
    this.on('mousedown', (event) => {
      ...
      this.focus(event)
      this._pressedIndex = this._indexFromEvent(event)
      this.dirty({ render: true })
      return false
    })
   ...
   */
  private _indexFromEvent(event: MouseEvent): number {
    const items = this.props.items
    if (items.length === 0) {
      return -1
    }
   ...
   */
  private _pushRoundAction(schema: NovaSchema, x: number, y: number, size: number, item: AdvancedItem, active: boolean): void {
    const background = item.color ?? this.props.accentColor ?? '#2563eb'
    this._pushRect(schema, x, y, size, size, active ? background : '#ffffff', 'item', 999, active ? background : '#cbd5e1')
    pushText(schema, item.icon ? '' : (item.label ?? '').slice(0, 1), x, y, size, size, this._textStyle({ color: active ? '#ffffff' : background, fontWeight: '800' }), { align: 'center' })
  }

  /**
   * Выполняет внутренний шаг pushRect для AdvancedComponent.
   */
  private _pushRect(schema: NovaSchema, x: number, y: number, width: number, height: number, background: string, part: string, radius = 0, borderColor?: string): void {
    const style = this._part(part)
    schema.push({
      type: 'rect',
      x,
      y,
   ...
   */
  private _pushStar(schema: NovaSchema, cx: number, cy: number, radius: number, color: string): void {
   ...
   */
  private _part(name: string): NovaUiPartStyle {
   ...
   */
  private _textStyle(patch: Partial<ReturnType<typeof resolveComponentTextStyle>> = {}): ReturnType<typeof resolveComponentTextStyle> {
    return {
