...
export function normalizeScrollAreaProps(props: ScrollAreaProps = {}): ScrollAreaResolvedProps {
  ...
  return {
    ...normalizeCommonProps(props, {
      width,
      height,
      background: 'var(--nova-scroll-area-background, #ffffff)',
      border: { color: 'var(--nova-scroll-area-border-color, #d6d9e2)', width: 1, radius: 8 },
      clip: true,
      trackColor: 'var(--nova-scroll-area-track-color, rgba(148,163,184,0.22))',
      thumbColor: 'var(--nova-scroll-area-thumb-color, rgba(71,85,105,0.7))',
    }),
