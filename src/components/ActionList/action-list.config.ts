...
export function normalizeActionListProps(props: ActionListProps = {}): ActionListResolvedProps {
  ...
  return { ...normalizeCommonProps(props, { width: 260, height: itemHeight * Math.max(1, props.items?.length ?? 4) + 12, background: 'var(--nova-action-list-background, #ffffff)', color: 'var(--nova-action-list-color, #172033)', border: { color: 'var(--nova-action-list-border-color, #cbd5e1)', width: 1, radius: 8 }, padding: { horizontal: 6, vertical: 6 }, activeBackground: 'var(--nova-action-list-active-background, #eff6ff)', cursor: { hover: 'pointer', pressed: 'pointer', disabled: 'not-allowed' } }), items: props.items ?? [], value: props.value, activeIndex: Math.max(0, Math.trunc(finiteNumber(props.activeIndex, 0))), itemHeight, loop: props.loop ?? true, selectable: props.selectable ?? true, parts: props.parts, onAction: props.onAction, onValueChange: props.onValueChange }
}
