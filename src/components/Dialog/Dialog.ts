...
export class Dialog<E extends EventList = Record<string, any>> extends NovaUiComponentNode<DialogResolvedProps, DialogApi, DialogProps, E> {
  ...
  render(): void {
    ...
    if (this.props.modal || this.props.backdrop) schema.push({ type: 'rect', x: 0, y: 0, width: this.width, height: this.height, styles: { background: this.resolveThemeValue('var(--nova-dialog-backdrop-background, rgba(15,23,42,0.38))') } })
