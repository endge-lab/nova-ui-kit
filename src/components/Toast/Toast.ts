...
import type { ToastApi, ToastItem, ToastProps, ToastRegionApi, ToastRegionProps, ToastRegionResolvedProps, ToastResolvedProps } from '@/components/Toast/toast.types'
import { reconcileNovaTemplateChildren } from '@endge/nova'
import { normalizeToastProps, normalizeToastRegionProps, TOAST_NODE_DESCRIPTOR, TOAST_REGION_NODE_DESCRIPTOR } from '@/components/Toast/toast.config'
...
export class Toast<E extends EventList = Record<string, any>> extends NovaUiComponentNode<ToastResolvedProps, ToastApi, ToastProps, E> {
  ...
  render(): void {
    ...
    if (this.props.closeButton) {
      pushText(schema, '×', this.width - 32, 8, 20, 20, { ...textStyle, fontSize: 19, fontWeight: '700' }, { align: 'center' })
    }
    if (this.props.progress !== undefined) {
      schema.push({ type: 'rect', x: 0, y: this.height - 3, width: this.width * this.props.progress, height: 3, styles: { background: this.resolveThemeValue('var(--nova-toast-progress-background, #2563eb)') } })
    }
...
export class ToastRegion<E extends EventList = Record<string, any>> extends NovaUiComponentNode<ToastRegionResolvedProps, ToastRegionApi, ToastRegionProps, E> {
  ...
  update(): void {
    this.visibleItems().forEach((_item, index) => {
      const node = this.toastNodes[index]; if (node) { applyNodeLayoutRect(node as NovaNode<any>, { x: this.resolveX(), y: this.resolveY(index), width: 320, height: 84 }) }
    })
  }
  ...
  protected override onPropsChanged(changedKeys: Array<keyof ToastRegionResolvedProps>): void {
    this.props = normalizeToastRegionProps(this.props); if (changedKeys.includes('items')) { this.reconcileToasts() } this.applyCommonPropsChanged(changedKeys)
  }
  ...
  private syncTimers(): void {
    if (!this.props.autoDismiss) {
      ...
      if (!this.timers.has(item.id) && (item.duration ?? 4500) > 0) {
        this.timers.set(item.id, window.setTimeout(() => this.dismiss(item.id), item.duration ?? 4500))
      }
    }
  }

  private dismiss(id: string, event?: Event): void {
    const item = this.props.items.find(current => current.id === id); if (!item) { return } const timer = this.timers.get(id); if (timer) { window.clearTimeout(timer) } this.timers.delete(id); this.setProps({ items: this.props.items.filter(current => current.id !== id) }); this.props.onDismiss?.(item, event)
  }

  private clear(): void {
    for (const timer of this.timers.values()) {
      window.clearTimeout(timer)
    } this.timers.clear(); this.setProps({ items: [] })
  }
