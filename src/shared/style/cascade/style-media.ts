...
export function resolveNovaUiResponsiveVariant(
  ...
  if (variant !== 'sm' && variant !== 'md' && variant !== 'lg') {
    return null
  }
...
export function matchesNovaUiMediaQuery(
  ...
  for (const feature of query.features) {
    if (feature.name === 'min-width' && context.width < feature.value) {
      return false
    }
    if (feature.name === 'max-width' && context.width > feature.value) {
      return false
    }
    if (feature.name === 'min-height' && context.height < feature.value) {
      return false
    }
    if (feature.name === 'max-height' && context.height > feature.value) {
      return false
    }
  }
