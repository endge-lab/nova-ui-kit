...
export function resolveNovaUiClassUtilities(className: unknown): NovaUiStyleDeclarations {
  ...
  if (!source) {
    return { mask: NovaUiStyleMask.None }
  }
...
function applyUtilityToken(target: NovaUiStyleDeclarations, rawToken: string): void {
  ...
  if (token === 'grow') {
    return setLayout(target, { flexGrow: 1 })
  }
  if (token === 'grow-0') {
    return setLayout(target, { flexGrow: 0 })
  }
  if (token === 'shrink') {
    return setLayout(target, { flexShrink: 1 })
  }
  if (token === 'shrink-0') {
    return setLayout(target, { flexShrink: 0 })
  }
  if (token === 'flex-1') {
    return setLayout(target, { flexGrow: 1, flexShrink: 1, flexBasis: 0 })
  }
  if (token === 'basis-auto') {
    return setLayout(target, { flexBasis: 'auto' })
  }
  if (token === 'w-full') {
    return setLayout(target, { width: 'fill' })
  }
  if (token === 'h-full') {
    return setLayout(target, { height: 'fill' })
  }
  if (token === 'min-w-0') {
    return setLayout(target, { minWidth: 0 })
  }
  if (token === 'min-h-0') {
    return setLayout(target, { minHeight: 0 })
  }
  ...
  if (arbitraryWidth !== undefined) {
    return setLayout(target, { width: arbitraryWidth })
  }

  const arbitraryHeight = parseArbitraryLayoutValue(token, 'h')
  if (arbitraryHeight !== undefined) {
    return setLayout(target, { height: arbitraryHeight })
  }

  const arbitraryBasis = parseArbitraryLayoutValue(token, 'basis')
  if (arbitraryBasis !== undefined) {
    return setLayout(target, { flexBasis: arbitraryBasis })
  }

  const numericWidth = parseNumericUtility(token, 'w')
  if (numericWidth !== undefined) {
    return setLayout(target, { width: numericWidth })
  }

  const numericHeight = parseNumericUtility(token, 'h')
  if (numericHeight !== undefined) {
    return setLayout(target, { height: numericHeight })
  }

  const numericMinWidth = parseNumericUtility(token, 'min-w')
  if (numericMinWidth !== undefined) {
    return setLayout(target, { minWidth: numericMinWidth })
  }

  const numericMinHeight = parseNumericUtility(token, 'min-h')
  if (numericMinHeight !== undefined) {
    return setLayout(target, { minHeight: numericMinHeight })
  }

  const numericGap = parseNumericUtility(token, 'gap')
  if (numericGap !== undefined) {
    return setLayout(target, { gap: numericGap })
  }

  const numericGapX = parseNumericUtility(token, 'gap-x')
  if (numericGapX !== undefined) {
    return setLayout(target, { columnGap: numericGapX })
  }

  const numericGapY = parseNumericUtility(token, 'gap-y')
  if (numericGapY !== undefined) {
    return setLayout(target, { rowGap: numericGapY })
  }
